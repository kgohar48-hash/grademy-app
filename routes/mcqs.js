...
router.get("/dashboard/mcqs/:category/:id", middelware.isLoggedIn,(req,res)=>{
    res.render("dashboard/mcqs/display",{category : req.params.category , id:req.params.id})
})
// send mcq
...
router.post("/mcq/:id/comment",(req,res)=>{
    ...
    Mcq.findById(req.params.id,(err,foundMcq)=>{
        if(err || !foundMcq){
            ...
            console.log("MCQ :",foundMcq.question)
            console.log("Comment : ",newCommnet)
        }
    })
})
// api for display correct incorrect mcqs
router.get("/mcqsattempted/:category/:id", middelware.isLoggedIn,(req,res)=>{
