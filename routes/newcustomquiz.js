...
router.get("/customquiz/newquiz",middelware.isLoggedIn ,function(req,res){
	if(!req.user.isAcademy){
		if(req.user.isPaid || req.user.isPaidPlus || req.user.score.attempted < 100 || trialOffer){
...
router.post("/dashboard/newcustomquiz" ,middelware.isLoggedIn ,async function(req,res){
	if(req.user.isPaid || req.user.isPaidPlus || req.user.score.attempted < 100 || req.user.isAcademy || trialOffer){
		var mcqsToBeAdded = []
...
router.get("/dashboard/newcustomquiz/view/start/:id",middelware.isLoggedIn , function(req,res){
	newCustomQuiz.findById(req.params.id , (err , foundQuiz)=>{
		if(err || !foundQuiz){
			...
			if(req.user.isPaid || req.user.isPaidPlus || foundQuiz.shareWith != "public" || req.user.score.attempted < 200 || trialOffer){
	...
	newCustomQuiz.findById(req.params.id,(err,foundQuiz)=>{
		if(err || !foundQuiz){
			...
			if(foundQuiz.shareWith == "competitive"){
				...
				if(req.user.isPaid || req.user.isPaidPlus || foundQuiz.shareWith == "free" || req.user.score.attempted < 100 || trialOffer){
